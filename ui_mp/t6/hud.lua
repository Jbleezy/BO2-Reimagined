...
function HUD_SetupEventHandlers_Zombie(HUDWidget)
	...
	HUDWidget:registerEventHandler("get_dvar", HUD_GetDvar)
end
...
CoD.DeadSpectate.UpdateVisibility = function(DeadSpectateWidget, ClientInstance)
	if DeadSpectateWidget.forceShowDeadSpectateHud and UIExpression.IsVisibilityBitSet(ClientInstance.controller, CoD.BIT_UI_ACTIVE) == 0 then
...
function HUD_FirstSnapshot_Zombie(HUDWidget, ClientInstance)
	...
	Widget:addElement(LUI.createMenu.TombstoneWaypointArea(ClientInstance.controller))
	...
	Widget:addElement(LUI.createMenu.PlayerEnemyNearbyArea(ClientInstance.controller))
